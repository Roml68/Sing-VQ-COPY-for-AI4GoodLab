...
def plot_pose(
    ...
    if poses.shape[-2] != 61:
        raise ValueError(
            f"Expected 61 keypoints, but got {poses.shape[-2]}"
        )
    ...
    for p, pose_ in enumerate(poses):
        ...
        # BODY (0–18)
        ax.scatter(
            pose[:19, 0], pose[:19, 1], pose[:19, 2],
            color="red", marker="o", s=s
        )

        # RIGHT HAND (19–39)
        ax.scatter(
            pose[19:40, 0], pose[19:40, 1], pose[19:40, 2],
            color="blue", marker="o", s=s
        )

        # LEFT HAND (40–60)
        ax.scatter(
            pose[40:61, 0], pose[40:61, 1], pose[40:61, 2],
            color="green", marker="o", s=s
        )
        ...
        # ========================
        if connections is not None:
            for k, (x, y) in enumerate(connections):
                if x >= 61 or y >= 61:
